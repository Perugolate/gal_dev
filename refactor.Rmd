...
countData <- read_tsv("gene_counts.tsv.gz", col_names=TRUE) %>% select(-X34)
rownames(countData) <- countData$gene
countData <- select(countData, -gene) %>% round
# Note, chose not to implement rowSums filter because it eliminates less than 10% rows.
# Read in sample descriptions and remove library 34 metadata.
colData <- read.table("meta", header=FALSE) %>% filter(V1 != "X34")
...
rld@colData@listData$stage <- factor(rld@colData@listData$stage, levels=c("one", "two", "three", "five", "six", "seven"))
```

## DE gene heatmap

Note `plotPCA` calls prcomp. Scaling makes no difference (I checked).
```{r}
...
# Plot cluster means
ggplot(devExpCl2, aes(x=stage, y=exp, group=cluster)) + geom_line() + facet_wrap(~cluster, ncol=5, scales="free_y") + ylab("Expression") + xlab("Stage") + ggtitle("Gene expression across larval-pupal moult") + geom_ribbon(aes(ymin=exp-ci, ymax=exp+ci), alpha=0.2)
```

## Plot PCs by stage

```{r}
pca2 <- prcomp(t(select(res_rld, -gene)), scale=FALSE)
...
pca2sum <- select(pca2x, PC1, PC2, PC3, PC4, stage) %>%
    gather(PC, value, -stage) %>%
    group_by(PC, stage) %>%
    summarize(val=mean(value), ci=std.error(value)*1.96)
ggplot(pca2sum, aes(x=stage, y=val, group=PC)) + geom_line() + facet_wrap(~PC, ncol=5, scales="free_y") + ylab("Expression") + xlab("Stage") + ggtitle("Gene expression across larval-pupal moult") + geom_ribbon(aes(ymin=val-ci, ymax=val+ci), alpha=0.2)
```
...
rsqll <- filter(sqll, q < 0.05) %>% group_by(gene) %>% filter(rev(rank(abs(loading)))==1)
...
wExpPca <- merge(select(wExp, -cluster), rsqll, by="gene")
# plot mean expression stage for each cluster.
select(wExpPca, -pc, -loading, -aloading, -sign, -q) %>% gather(stage, expression, -gene, -cluster) %>% 
    group_by(cluster, stage) %>% summarize(exp=mean(expression), ci=std.error(expression)*1.96) %>%
