...
names (l2fChanges) <- lapply(names(contr1), function(x) {paste(contr1[,x], collapse="_vs_")})
```
...
# Plot cluster means
ggplot(devExpCl2, aes(x=stage, y=exp, group=cluster)) + geom_line() + facet_wrap(~cluster, ncol=5, scales="free_y") + ylab("Expression") + xlab("Stage") + ggtitle("Gene expression across larval-pupal moult") + geom_ribbon(aes(ymin=exp-sd, ymax=exp+sd), alpha=0.2)
# Plot each gene by cluster
```
