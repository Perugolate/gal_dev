...
countData <- read_tsv("gene_counts.tsv.gz", col_names = TRUE) %>%
  dplyr::select(-X34)
...
colData$stage <- factor(colData$stage, levels = c("one", "two", "three",
                                                  "five", "six", "seven"))
...
res <- results(dds, alpha = 0.05, tidy = TRUE, filterFun = ihw)
```
## Check GALME gene
...
ggplot(p82174, aes(x = stage, y = count, color = batch)) + geom_point(size = 5) +
       ...
       xlab("Developmental stage") + ylab("Lysozyme p82174 gene expression (normalized count)") +
...
foldchanges <- lapply(names(contr), function(x) {
  results(dds, alpha = 0.05, filterFun = ihw, parallel = TRUE,
  contrast = c("stage",contr[,x])) %>%
...
colData2 <- unite(colData2, sample_stage, sample, stage, remove = FALSE)
colnames(res_rld) <- c(colData2$sample_stage, "gene")

#colnames(res_rld) <- c(paste(colData[,1] , colData$stage, sep="_"))
...
```r
write.table(dplyr::select(devExpCw, -gene), file = "genes.csv", quote = FALSE,
            row.names = FALSE, sep = ",")
```
...
foo = np.loadtxt(open("genes.csv", "r"), delimiter=",", skiprows=1)
...
A <- pca_scaled(D)
L <- A[["factor.loading"]]
colnames(L) <- c("gene", paste("PC", c(1:(ncol(L)-1)), sep = ""))
Ll <- gather(L, pc, loading, -gene)
Q <- A[["q.value"]]
colnames(Q) <- c("gene", paste("PC", c(1:(ncol(L)-1)), sep = ""))
...
gather(sigSac, stage, exp, -gene) %>% group_by(stage) %>% 
  summarize(expression=mean(exp)) %>% ggplot(aes(x=stage, y=expression)) + 
  geom_point() + scale_x_discrete(limits=c("one","two","three","five","six","seven"))
```
...
perl $TRINITY_HOME/util/abundance_estimates_to_matrix.pl --est_method RSEM  --cross_sample_norm none --out_prefix gene rsem/X*
# filter to retain top isoforms
perl $TRINITY_HOME/util/filter_low_expr_transcripts2.pl -m gene.TPM.not_cross_norm -t gal_dev.fasta --highest_iso_only --trinity_mode > gal_dev_fil.fasta 
# translate the transcripts
TransDecoder.LongOrfs -m 75 -t gal_dev_fil.fasta
# prepare orthofinder input
mkdir ortho && cd ortho
# needs fasta extensions
ln -s ../Gnomon_prot.fsa Gnomon_prot.fasta
ln -s ../gal_dev_fil.fasta.transdecoder_dir/longest_orfs.pep longest_orfs.fasta
cd ../ && orthofinder -f ortho -t 12
```
